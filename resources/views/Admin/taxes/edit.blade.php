        ...
        <div class="col-12">
            <div class="card card-rounded shadow-sm">
                <div class="card-header">
                    <h4 class="m-0">Edit Tax: {{ $tax->name }}</h4>
                </div>
                <div class="card-body">
                    <form method="POST" action="{{ route('superadmin.taxes.update', $tax) }}">
                        @csrf
                        @method('PUT')
                            ...
                            <div class="col-md-6">
                                <div class="mb-3">
                                    ...
                                    <input type="text" class="form-control" id="code" name="code" 
                                           value="{{ old('code', $tax->code) }}" required placeholder="e.g., VAT, GST">
                                    @error('code')
                                        <div class="text-danger small">{{ $message }}</div>
                                    @enderror
                                </div>
                            ...
                            <div class="col-md-6">
                                <div class="mb-3">
                                    ...
                                    <input type="number" class="form-control" id="rate" name="rate" 
                                           value="{{ old('rate', $tax->rate) }}" required step="0.01" min="0" max="100" placeholder="e.g., 12.5">
                                    @error('rate')
                                        <div class="text-danger small">{{ $message }}</div>
                                    @enderror
                                </div>
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control" id="description" name="description" rows="3" 
                                      placeholder="Optional description for this tax">{{ old('description', $tax->description) }}</textarea>
                            @error('description')
                                <div class="text-danger small">{{ $message }}</div>
                            @enderror
                        </div>
