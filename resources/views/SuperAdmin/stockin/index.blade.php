...
<style>
    ...
    .sp-product-link:hover { color:var(--navy);text-decoration:underline; }
    ...
    .sp-badge-blue { background:rgba(25,118,210,0.12);color:#1e40af; }
        ...
        <div class="sp-wrap">
            ...
            <div class="sp-page-head">
                <div class="sp-ph-left">
                    <div class="sp-ph-icon"><i class="fas fa-boxes"></i></div>
                    <div>
                        <div class="sp-ph-crumb">Operations</div>
                        <div class="sp-ph-title">Stock In</div>
                        <div class="sp-ph-sub">Track and manage all stock movements</div>
                    </div>
                </div>
            ...
            <div class="sp-card">
                ...
                <div class="sp-card-head">
                    <div class="sp-card-head-title">
                        <i class="fas fa-list"></i> Stock Transactions
                    </div>
                    <span class="sp-c-badge">{{ $stockIns->total() }} records</span>
                </div>
                    ...
                    <table class="sp-table">
                        ...
                        <tbody>
                            @forelse($stockIns as $stock)
                                <tr>
                                    <td>
                                        <span class="sp-movement-id">
                                            <i class="fas fa-receipt" style="font-size:9px;opacity:.6;"></i>
                                            {{ $stock->reference_number }}
                                        </span>
                                    </td>
                                    <td>
                                        <span style="font-weight:600;">{{ $stock->branch_name ?? 'N/A' }}</span>
                                    </td>
                                    <td>
                                        <a href="{{ route('superadmin.purchases.show', $stock->purchase_id) }}" class="sp-product-link">
                                            <i class="fas fa-file-invoice me-1" style="font-size:11px;opacity:.6;"></i>
                                            {{ $stock->purchase_reference ?? 'N/A' }}
                                        </a>
                                    </td>
                                    <td>
                                        <span style="font-weight:600;">{{ $stock->supplier_name ?? 'N/A' }}</span>
                                    </td>
                                    <td>
                                        <span class="sp-quantity">{{ number_format($stock->total_quantity ?? 0, 2) }}</span>
                                        <span style="color:var(--muted);font-size:12px;"> units</span>
                                    </td>
                                    <td>
                                        <span class="sp-badge sp-badge-green">
                                            <i class="fas fa-circle me-1" style="font-size:7px;"></i> {{ ucfirst($stock->status ?? 'N/A') }}
                                        </span>
                                    </td>
                                    <td>
                                        <span style="color:var(--text);font-weight:500;">
                                            <i class="fas fa-calendar-alt me-1" style="font-size:11px;opacity:.6;"></i>
                                            {{ !empty($stock->stock_in_date) ? \Carbon\Carbon::parse($stock->stock_in_date)->format('M d, Y') : 'N/A' }}
                                        </span>
                                    </td>
                                    <td>
                                        <a href="{{ route('superadmin.stockin.transaction', $stock->id) }}" class="sp-tbl-btn">
                            ...
                            @empty
                                <tr>
                                    <td colspan="8" class="empty-row">
                                        <i class="fas fa-inbox me-2"></i>No stock records found.
                                    </td>
                                </tr>
                            @endforelse
                        </tbody>
