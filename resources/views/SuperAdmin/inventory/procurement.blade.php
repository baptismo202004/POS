...
.proc-banner-icon{font-size:22px;color:var(--amber);}
.proc-banner-text{flex:1;font-size:13px;color:var(--text);}
.proc-banner-text strong{color:#92400e;}

/* KPI strip */
.proc-kpis{display:grid;grid-template-columns:repeat(auto-fit,minmax(150px,1fr));gap:10px;margin-bottom:18px;}
...
.proc-kpi.p::before{background:var(--purple);}
...
/* Stock alert strip (reused from stock management) */
.proc-stock-alerts{display:flex;gap:8px;flex-wrap:wrap;margin-bottom:18px;}
...
.sp-card-head{padding:13px 20px;background:linear-gradient(135deg,var(--navy),var(--blue));display:flex;align-items:center;justify-content:space-between;position:relative;overflow:hidden;}
...
.sp-card-head-title{font-family:'Nunito',sans-serif;font-size:14px;font-weight:800;color:#fff;display:flex;align-items:center;gap:8px;position:relative;z-index:1;}
...
.sp-search-input{padding:8px 13px 8px 34px;border-radius:10px;border:1.5px solid var(--border);font-size:13px;background:var(--card);color:var(--text);outline:none;width:240px;transition:border-color .18s,box-shadow .18s;}
...
.sp-table tbody td{padding:12px 15px;font-size:13px;color:var(--text);border-bottom:1px solid rgba(25,118,210,0.06);vertical-align:middle;}
        ...
        <div class="sp-wrap">
            ...
            <div class="sp-page-head">
                <div class="sp-ph-left">
                    ...
                    <div>
                        <div class="sp-ph-crumb">Inventory · Stock Management</div>
                ...
                <div class="sp-ph-actions">
                    <form method="GET" action="{{ route('superadmin.inventory.procurement') }}" class="d-flex gap-2 align-items-center">
                        <div class="sp-search-wrap">
                            <i class="fas fa-search"></i>
                            <input type="text" name="search" class="sp-search-input" placeholder="Search product, branch, supplier..." value="{{ $search }}">
                        </div>
                        <button type="submit" class="btn btn-primary btn-sm">Search</button>
                        @if($search)
                            <a href="{{ route('superadmin.inventory.procurement') }}" class="btn btn-outline-secondary btn-sm">Clear</a>
                        @endif
                    </form>
                    <a href="{{ route('superadmin.inventory.stock-management') }}" class="btn btn-outline-secondary btn-sm">
                        <i class="fas fa-boxes-stacked me-1"></i> Stock Management
                    </a>
                    <a href="{{ route('superadmin.purchases.create') }}" class="btn btn-warning btn-sm fw-bold">
            ...
            <div class="proc-banner">
                <div class="proc-banner-icon"><i class="fas fa-exclamation-triangle"></i></div>
                <div class="proc-banner-text">
                    ...
                    @endif
                    Create a purchase order to resolve these.
                </div>
                <a href="{{ route('superadmin.purchases.create') }}" class="btn btn-warning btn-sm fw-bold">
            ...
            {{-- Stock alert strip (reused from stock management) --}}
            <div class="proc-stock-alerts">
                <a href="{{ route('superadmin.inventory.stock-management', ['stock_levels[]' => 'out_of_stock']) }}" class="proc-sa red">
                    <i class="fas fa-times-circle"></i>
                    <span class="proc-sa-count">{{ $stockStats['out_of_stock'] ?? 0 }}</span>
                    <span>Out of Stock</span>
                </a>
                <a href="{{ route('superadmin.inventory.stock-management', ['stock_levels[]' => 'critical_stock']) }}" class="proc-sa red">
                    <i class="fas fa-exclamation-circle"></i>
                    <span class="proc-sa-count">{{ $stockStats['critical_stock'] ?? 0 }}</span>
                    <span>Critical</span>
                </a>
                <a href="{{ route('superadmin.inventory.stock-management', ['stock_levels[]' => 'low_stock']) }}" class="proc-sa amber">
                    <i class="fas fa-exclamation-triangle"></i>
                    <span class="proc-sa-count">{{ $stockStats['low_stock'] ?? 0 }}</span>
                    <span>Low Stock</span>
                </a>
                <a href="{{ route('superadmin.inventory.stock-management', ['stock_levels[]' => 'in_stock']) }}" class="proc-sa green">
                    <i class="fas fa-check-circle"></i>
                    <span class="proc-sa-count">{{ $stockStats['in_stock'] ?? 0 }}</span>
                    <span>In Stock</span>
                </a>
                <a href="{{ route('superadmin.inventory.stock-management') }}" class="proc-sa blue">
                    <i class="fas fa-boxes-stacked"></i>
                    <span class="proc-sa-count">{{ $stockStats['total_products'] ?? 0 }}</span>
            ...
            {{-- KPI strip --}}
            <div class="proc-kpis">
                <div class="proc-kpi r">
                    <div class="proc-kpi-label">Fully Blocked</div>
                    <div class="proc-kpi-value" style="color:var(--red);">{{ $fullyBlocked }}</div>
                    <div class="proc-kpi-sub">zero stock, can't fulfill</div>
                </div>
                <div class="proc-kpi a">
                    <div class="proc-kpi-label">Partially Fulfillable</div>
                    <div class="proc-kpi-value" style="color:var(--amber);">{{ $partiallyFulfillable }}</div>
                    <div class="proc-kpi-sub">some stock, still short</div>
                </div>
                <div class="proc-kpi b">
                    <div class="proc-kpi-label">Product Lines</div>
                    <div class="proc-kpi-value">{{ $totalPendingProducts }}</div>
                    <div class="proc-kpi-sub">needing procurement</div>
                </div>
                <div class="proc-kpi a">
                    <div class="proc-kpi-label">Total Pending Units</div>
                    <div class="proc-kpi-value">{{ number_format($totalPendingUnits, 0) }}</div>
                    <div class="proc-kpi-sub">ordered, not fulfilled</div>
                </div>
                <div class="proc-kpi r">
                    <div class="proc-kpi-label">Still Needed</div>
                    <div class="proc-kpi-value" style="color:var(--red);">{{ number_format($totalStillNeeded, 0) }}</div>
                    <div class="proc-kpi-sub">units to purchase</div>
                </div>
                <div class="proc-kpi b">
                    <div class="proc-kpi-label">Affected Orders</div>
                    <div class="proc-kpi-value">{{ number_format($totalOrders, 0) }}</div>
                    <div class="proc-kpi-sub">sales waiting</div>
                </div>
            </div>
            ...
            <div class="sp-card">
                <div class="sp-card-head">
                    <div class="sp-card-head-title">
                        <i class="fas fa-truck-loading"></i> Items Needing Procurement
                    </div>
                    ...
                    <table class="sp-table">
                        <thead>
                            <tr>
                                ...
                                <th class="text-end">Still Needed</th>
                                <th>Fulfillment</th>
                                <th>Oldest Order</th>
                                <th>Status</th>
                                <th>Action</th>
                        ...
                        <tbody>
                            ...
                            @php
                                $pct = $row->total_pending_qty > 0
                                    ? min(100, round($row->can_fulfill / $row->total_pending_qty * 100))
                                    : 0;
                            ...
                            <tr>
                                <td>
                                    <div style="font-weight:700;font-size:13px;">{{ $row->product_name }}</div>
                                    <div style="font-size:11px;color:var(--muted);">
                                        {{ $row->barcode ?? '' }}
                                        @if($row->model_number) · {{ $row->model_number }} @endif
                                    </div>
                                    <div style="font-size:11px;color:var(--muted);">{{ $row->category_name }} · {{ $row->brand_name }}</div>
                                </td>
                                <td style="font-size:12.5px;">{{ $row->branch_name }}</td>
                                <td style="font-size:12.5px;">{{ $row->supplier_name }}</td>
                                <td class="text-center">
                                    <span class="bp bp-blue">{{ $row->order_count }}</span>
                                </td>
                                <td class="text-end" style="font-weight:700;">{{ number_format($row->total_pending_qty, 0) }}</td>
                                <td class="text-end" style="font-weight:700;color:{{ $row->current_stock > 0 ? 'var(--green)' : 'var(--red)' }};">
                                    {{ number_format($row->current_stock, 0) }}
                                </td>
                                <td class="text-end" style="font-weight:800;color:var(--red);">
                                    {{ $row->still_needed > 0 ? number_format($row->still_needed, 0) : '—' }}
                                </td>
                                <td style="min-width:100px;">
                                    <div class="proc-bar-wrap">
                                        <div class="proc-bar" style="width:{{ $pct }}%;background:{{ $barColor }};"></div>
                                    </div>
                                    <div style="font-size:10.5px;color:var(--muted);margin-top:2px;">{{ $pct }}% can fulfill</div>
                                </td>
                                <td style="font-size:11px;color:var(--muted);white-space:nowrap;">
                                    ...
                                    @if($age > 7)
                                        <span class="bp bp-red ms-1">{{ $age }}d</span>
                                    @elseif($age > 2)
                                        <span class="bp bp-amber ms-1">{{ $age }}d</span>
                                    @endif
                                </td>
                                <td><span class="bp {{ $statusClass }}">{{ $statusLabel }}</span></td>
                                <td>
                                    <a href="{{ route('superadmin.purchases.create') }}"
                                       class="btn btn-warning btn-sm fw-bold"
                                       title="Create purchase for {{ $row->product_name }}">
                            ...
                            @empty
                            <tr>
                                <td colspan="11" class="text-center" style="padding:48px;color:var(--muted);">
                                    <i class="fas fa-check-circle" style="font-size:36px;color:var(--green);opacity:.4;display:block;margin-bottom:10px;"></i>
                                    <div style="font-weight:700;font-size:14px;">All orders are fulfilled</div>
